	...
	private function tryUseTotem() : bool{
		...
		if($isInOffhand || $this->getInventory()->getItemInHand() instanceof Totem){
			...
			if(!$event->isCancelled()){
				...
				if($isInOffhand){
					$this->getOffhandInventory()->setItemInOffhand(ItemItem::get(BlockIds::AIR));
				}else{
					$this->getInventory()->setItemInHand(ItemItem::get(BlockIds::AIR));
				}
	...
	public function saveNBT(){
		...
		if($this->inventory !== null){
			...
			for($slot = 100; $slot < 104; ++$slot){
				...
				if($item instanceof ItemItem and $item->getId() !== BlockIds::AIR){
					$this->namedtag->Inventory[$slot] = $item->nbtSerialize($slot);
				}
			}
		}
