	...
	private function readPacket($client, ?int &$requestID, ?int &$packetType, ?string &$payload){
		...
		if($d === false){
			...
			if($err !== SOCKET_ECONNRESET){
				$this->logger->debug("Ошибка подключения с $ip $port: " . trim(socket_strerror($err)));
			}
			return false;
		}
		if(strlen($d) !== 4){
			if($d !== ""){ //empty data is returned on disconnection
				$this->logger->debug("Усеченный пакет от $ip $port (хотите 4 байта, есть " . strlen($d) . "), отключение");
			}
			return false;
		}
		$size = Binary::readLInt($d);
		if($size < 0 or $size > 65535){
			$this->logger->debug("Пакет с заголовком слишком большой длины $size от $ip $port, отключение");
			return false;
		}
		...
		if(strlen($buf) !== $size){
			$this->logger->debug("Усеченный пакет от $ip $port (хотеть $size байты, есть " . strlen($buf) . "), отключение");
			return false;
		}
						...
						switch($packetType){
							case 3: //Login
								...
								if($payload === $this->password){
									...
									$this->logger->info("Успешное подключение Rcon от: /$addr:$port");
